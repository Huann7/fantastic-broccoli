...
if page == "项目介绍":
    ...
    with col2:
        st.image("77.png", caption="学生数据分析示意图",width=500)
...
# ---------- 3. 成绩预测页面 ----------
    ...
    with col17:
        ...
        midterm_score = st.slider("期中考试分数", 0.0, 100.0, 60.0, 0.1)
    ...
    if st.button("预测期末成绩", type="primary"):
        ...
        if predicted_score >= 60:
            ...
            st.image("999.jpg", 
                     caption="恭喜你，预测结果显示你会及格！", use_column_width=True)
            st.info("学习建议：保持当前学习节奏，考前针对性复习即可稳定发挥。")
        else:
            st.error(f"预测期末成绩: {predicted_score:.1f}分")
            st.image("888.jpg", 
                     caption="需要加油哦，预测结果显示成绩未达标。", use_column_width=True)
            st.warning("学习建议：增加每周学习时长至15小时以上，重点攻克期中考试薄弱知识点，作业完成率提升至0.85以上。")
